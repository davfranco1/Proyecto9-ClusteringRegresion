...
def detectar_orden_cat(df, lista_cat, var_respuesta):
    ...
    try:
        for categorica in lista_cat:
            ...
            if p < 0.05:
                print(f"La variable {categorica} TIENE orden, quiere decir que tiene relación con la VR. ✅")
                var_ordenadas.append(categorica)
            else:
                print(f"La variable {categorica} NO TIENE orden, quiere decir que NO tiene relación con la VR. ❌")
                var_noordenadas.append(categorica)
                
            print("_________________________ \n")
        
        print(f"Las variables ordenadas (relacionadas) son: {var_ordenadas}")
        print(f"Las variables no ordenadas (no relacionadas) son: {var_noordenadas}")
        print("_________________________ \n")
        
        return var_ordenadas, var_noordenadas
    
    except Exception as e:
        print(f"Error in detectar_orden_cat: {e}")
        return None, None

import pandas as pd
import matplotlib.pyplot as plt

def plot_top5_num(df, figsize=(15, 10)):
    """
    Plots the top 5 values for each numeric column in a DataFrame.
    ...
    fig, axes = plt.subplots(num_columns, 1, figsize=figsize, constrained_layout=True)
    ...
    for i, col in enumerate(numeric_columns):
        top5 = df.nlargest(5, col)  # Get the top 5 rows for the current column
        axes[i].bar(top5.index, top5[col], tick_label=top5.index)
